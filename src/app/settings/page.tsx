...
export default function SettingsPage() {
  ...
  // Layouts & Fields Customization State matching Screenshot 1
  const [layoutTab, setLayoutTab] = useState<"NEW" | "AVAILABLE">("NEW");
  const [taskLayoutFields, setTaskLayoutFields] = useState<any[]>([
    { id: "owner", name: "Owner", type: "User Pick List", removable: false },
    { id: "associated_team", name: "Associated Team", type: "Multi Select", removable: true, highlighted: true },
    { id: "work_hours", name: "Work Hours", type: "Decimal", removable: true },
    { id: "status", name: "Status", type: "Pick List", removable: false, defaultValue: "Open" },
    { id: "start_date", name: "Start Date", type: "Date & Time", removable: true },
    { id: "due_date", name: "Due Date", type: "Date & Time", removable: true },
    { id: "duration", name: "Duration", type: "Decimal", removable: true },
    { id: "priority", name: "Priority", type: "Pick List", removable: true },
    { id: "pct", name: "Completion Percentage", type: "Percentage", removable: true },
    { id: "rate", name: "Rate Per Hour", type: "Decimal", removable: true },
    { id: "email", name: "Email", type: "Email", removable: true },
    { id: "tags", name: "Tags", type: "Multi Select", removable: true },
    { id: "reminder", name: "Reminder", type: "Pick List", removable: true },
    { id: "recurrence", name: "Recurrence", type: "Pick List", removable: true },
  ]);

  // Email Templates State matching Screenshot 2
  const [emailLayoutName, setEmailLayoutName] = useState("Digital Twin Solutions");
  const [emailSubject, setEmailSubject] = useState("Update on ${Task.GROUP_NAME} - Task ${Task.Number} Due ${Task.Due_Date}");
  const [emailBodyText, setEmailBodyText] = useState(
    "Please review the task and let us know if there are any changes or additional information needed. Your prompt attention to the upcoming deadline would be greatly appreciated.\n\nThank you for your continued effort and dedication."
  );
  ...
  const categories = [
    ...
    },
    {
      ...
      items: [
        { id: "PROJ_CONFIG", label: "Project Fields" },
        { id: "LAYOUTS_FIELDS", label: "Layouts and Fields" },
      ],
    },
    {
      group: "CUSTOMIZATION",
    ...
    },
    {
      group: "AUTOMATION",
      items: [
        { id: "AUTOMATION", label: "Workflow Rules" },
    ...
    <div className="flex h-screen w-screen bg-slate-50 font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-8 bg-slate-50/50 font-sans">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {/* LAYOUTS AND FIELDS SUB-VIEW matching Screenshot 1 */}
          {activeCategory === "LAYOUTS_FIELDS" && (
            <div className="space-y-6 animate-fadeIn font-sans text-xs">
              <div className="flex items-center justify-between border-b border-slate-200 bg-white p-4 rounded-lg shadow-2xs">
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Task Layouts and Fields</h3>
                  <p className="text-slate-500 text-xs">Customize task forms, add custom fields, and drag-and-drop field positions</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => alert("Layout saved successfully!")}
                    className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-md shadow-2xs cursor-pointer"
                  >
                    Save Layout
                  </button>
                  <button
                    onClick={() => setActiveCategory("PERSONAL_SETTINGS")}
              ...
              <div className="flex gap-6">
                {/* Left Fields Palette Palette matching Screenshot 1 */}
                <div className="w-80 bg-white border border-slate-200 rounded-xl p-4 shadow-2xs space-y-4 flex-shrink-0">
                  <div className="flex border-b border-slate-200 text-xs font-bold">
                    <button
                      onClick={() => setLayoutTab("NEW")}
                      className={`pb-2 px-3 border-b-2 ${layoutTab === "NEW" ? "border-rose-600 text-rose-600 font-bold" : "border-transparent text-slate-500"}`}
                    >
                      New Fields
                    </button>
                    <button
                      onClick={() => setLayoutTab("AVAILABLE")}
                      className={`pb-2 px-3 border-b-2 ${layoutTab === "AVAILABLE" ? "border-rose-600 text-rose-600 font-bold" : "border-transparent text-slate-500"}`}
                    >
                      Available Fields
                    </button>
                  </div>
                  ...
                  {layoutTab === "NEW" && (
                    <div className="space-y-3">
                        ...
                        {[
                          ...
                          <div
                            ...
                            onClick={() => {
                              setTaskLayoutFields([
                                ...taskLayoutFields,
                                { id: `field-${Date.now()}`, name: field, type: field, removable: true }
                              ]);
                            }}
                            className="p-2.5 bg-slate-50 border border-slate-200 rounded-lg hover:border-rose-400 hover:bg-rose-50/50 cursor-pointer flex items-center justify-between transition-colors shadow-2xs"
                          >
                      ...
                      <button
                        onClick={() => alert("Added new layout section")}
                        className="w-full py-2 bg-slate-100 border border-slate-300 rounded-md font-bold text-slate-700 hover:bg-slate-200 cursor-pointer"
                      >
                        + Add Section
                      </button>

                      <p className="text-center font-bold text-rose-600 text-[11px]">
                        Remaining Fields: 254
                      </p>
                    </div>
                  )}
                  ...
                  {layoutTab === "AVAILABLE" && (
                    <div className="space-y-2 text-slate-700 font-semibold">
                      {["Task Name", "Description", "Milestone", "Task List", "Predecessor"].map((f) => (
                        <div key={f} className="p-2.5 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-between">
                          <span>{f}</span>
                          <span className="text-xs text-rose-600 font-bold">Default</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {/* Right Layout Canvas Editor matching Screenshot 1 */}
                <div className="flex-1 bg-white border border-slate-200 rounded-xl p-6 shadow-2xs space-y-6">
                  <div className="border-b border-slate-200 pb-3 flex items-center justify-between">
                    <h4 className="font-bold text-slate-900 text-sm">Task Information Layout Canvas</h4>
                    <span className="text-xs text-slate-400">Associated Projects: None</span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {taskLayoutFields.map((field) => (
                      <div
                        key={field.id}
                        className={`p-3.5 rounded-lg border text-xs flex items-center justify-between transition-all ${
                          field.highlighted
                            ? "border-rose-500 bg-rose-50/30 ring-2 ring-rose-400/30 font-bold"
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 text-slate-400">
                          {field.removable && (
                            <button
                              type="button"
                              onClick={() => setTaskLayoutFields(taskLayoutFields.filter(f => f.id !== field.id))}
                  ...
                  <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-200">
                    <button
                      onClick={() => alert("Task layout saved successfully!")}
                      className="px-5 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-md shadow-2xs cursor-pointer"
                    >
                      Save Layout
                    </button>
                  </div>
          ...
          {/* EMAIL TEMPLATES SUB-VIEW matching Screenshot 2 */}
          {activeCategory === "EMAIL_TEMPLATES" && (
            <div className="space-y-6 animate-fadeIn font-sans text-xs max-w-4xl">
              <div className="border-b border-slate-200 pb-3">
                <h3 className="text-lg font-bold text-slate-900">Create an Email Template</h3>
                <p className="text-slate-500 text-xs">Configure automated notification email templates for task triggers</p>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-2xs space-y-4">
                <div>
                  ...
                  <input
                    ...
                    value={portalName}
                    readOnly
                    className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded font-semibold text-slate-700"
                  />
                </div>

                <div>
                  <label className="block font-bold text-slate-800 mb-1">Layout Name *</label>
                  <select
                    value={emailLayoutName}
                    onChange={(e) => setEmailLayoutName(e.target.value)}
                  ...
                  >
                    <option value="Digital Twin Solutions">Digital Twin Solutions</option>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="font-bold text-slate-800">Subject *</label>
                    <select
                      onChange={(e) => {
                        if (e.target.value) {
                          setEmailSubject(prev => prev + " " + e.target.value);
                        }
                      }}
                      className="text-xs font-bold text-orange-600 border border-orange-300 rounded px-2 py-1 bg-white cursor-pointer"
                    >
                      <option value="">Insert Placeholder ▾</option>
                      <option value="${Task.Associated_Team}">{"${Task.Associated_Team}"}</option>
                      <option value="${Task.Name}">{"${Task.Name}"}</option>
                      <option value="${Task.Number}">{"${Task.Number}"}</option>
                      <option value="${Task.Due_Date}">{"${Task.Due_Date}"}</option>
                  ...
                  <input
                    ...
                    className="w-full p-2.5 border border-slate-300 rounded font-mono font-semibold text-slate-800 focus:border-[#0070BA] focus:outline-none"
                  />
                </div>

                <div>
                  <label className="block font-bold text-slate-800 mb-2">Email Body *</label>
                  <textarea
                    rows={6}
                    value={emailBodyText}
                    onChange={(e) => setEmailBodyText(e.target.value)}
                    className="w-full p-3 border border-slate-300 rounded font-medium text-slate-800 focus:border-[#0070BA] focus:outline-none leading-relaxed"
                  />
                </div>
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-slate-200">
                  <button
                    ...
                    className="px-5 py-2 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-md shadow-2xs cursor-pointer"
                  >
                    Save Template
                  </button>
                  <button
          ...
          {/* EMAIL ALERTS SUB-VIEW */}
          {activeCategory === "EMAIL_ALERTS" && (
            <div className="space-y-6 animate-fadeIn font-sans text-xs max-w-4xl">
              <div className="border-b border-slate-200 pb-3 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Email Alerts</h3>
                  <p className="text-slate-500 text-xs">Configure automated email triggers to notify users and teams</p>
                </div>
                <button
                  onClick={() => alert("Created new Email Alert")}
                  className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-md cursor-pointer"
                >
                  + Add Email Alert
                </button>
              </div>
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs space-y-3">
                <div className="p-3 bg-slate-50 rounded border border-slate-200 flex items-center justify-between">
                  <div>
                    <h4 className="font-bold text-slate-900">Notify Associated Team on Task Assignment</h4>
                    <p className="text-slate-500 text-xs">Triggered when task Associated Team field is updated</p>
                  </div>
