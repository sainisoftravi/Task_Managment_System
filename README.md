...
- **Pixel-Accurate 3-Step Project Creation Flow & Row Context Menu (`projects/page.tsx`)**:
  - **Step 1: Template Gallery Modal (`template-gallery-modal.tsx`)**: Filter tabs (*All*, *Project Templates*, *Software/IT*, *Construction*, *Pharma*, *Marketing/Sales*), dropdown filter, search bar, **Blank Project Card** (dashed border with large `+` icon), category layout cards, `Do not show this again` toggle, and `[ Create Project ]` primary button.
  - **Step 2: "New Project" Form Configuration (`new-project-form-modal.tsx`)**: Title badge (*Standard Layout* + edit icon), required `Project Title*` input, Owner & Template dropdowns, Start & End Date pickers, `Make this a strict project` checkbox with info tooltip, `Project Group` with `+ Add new group` action link, Rich Text Editor toolbar (*Bold*, *Italic*, *Lists*, *Alignments*, *AI Assistant*), Business Hours & Layout selectors, multi-tag input, Roll-up accordion, module tab customizer pills (*Dashboard*, *Tasks*, *Users*, *Reports*, *Documents*, *Phases*, *Time Logs*, *Finance*, *Expense Claims*, *Bugs*, *Forums*, *Pages*, *Timesheet*), and `Private` vs `Public` access radio options. Submitting assigns unique numeric Project ID (e.g. `473238000000077082` / `DT-32`) and updates `localStorage`.
  - **Step 3: All Projects Table & Navigation (`projects/page.tsx`)**: Top navigation tabs (*Active Projects*, *Project Templates*, *Project Groups*, *Public Projects*, *Archived Projects*), sub-header toolbar (*All Projects ▾*, *List/Kanban/Gantt* view switcher, *Automation* button, *+ New Project* button), and full data table columns (*Actions ···*, *ID*, *Project Name*, *% Progress*, *Owner*, *Status*, *Tasks*, *Phases*, *Issues*, *Start Date*, *End Date*).
  - **Step 4: Row Context Menu (3-Dots Dropdown & Action Drawers)**:
    1. **Access Project**: Navigates tab to `/projects/[id]`.
    2. **Access Project in New Tab**: Opens workspace URL in new browser tab (`target="_blank"`).
    3. **View Details**: Opens slide drawer (`project-detail-drawer.tsx`) displaying metadata, owner, timeline, email alias, and copy link.
    4. **View Details in New Tab**: Opens detailed workspace view in new tab.
    5. **Copy Link**: Copies direct dynamic URL (`http://localhost:3000/projects/{id}`) with toast confirmation.
    6. **Color**: Opens color picker palette (`project-color-picker-modal.tsx`) to set project accent color badge.
    7. **Edit Project**: Re-opens Step 2 form pre-populated for editing.
    8. **Email Alias**: Shows inbound task email alias modal (`project-dt31@taskpmp.local`).
    9. **Change Layouts**: Opens layout switcher modal (`project-layout-switcher-modal.tsx`).
    10. **Move to Archive / Trash**: Confirmation dialog to archive/trash project.
- **Task List & Task Creation Engine (`create-task-list-modal.tsx`, `task-list-details-drawer.tsx`)**:
  - **New Task List Modal (`create-task-list-modal.tsx`)**: Form modal matching reference Screenshot 1 with required `Task List*` input (e.g. `01 POC TEST`), `Clone from a Project or Project Template ℹ` link, `Related Phase` selector, `Task List Flag ℹ` (`Internal`, `External`, `Public`), multi-tag input, and `Add` / `Add More` / `Cancel` action buttons.
  - **Task List Grouping View (`task-list-view.tsx`)**: Group by Task List (`Group By: Task List ▾`) rendering task list section headers (e.g. `01 POC TEST (1)`) with task count badges and popout drawer icons `[↗]`.
  - **TaskList Details Side Drawer (`task-list-details-drawer.tsx`)**: Slide drawer matching reference Screenshots 4 & 5 with circular progress chart (`0%`), `1 Open / 0 Closed` metrics, `Tasks` / `Comments` / `Chart View` sub-tabs, full task table (e.g. `OD1-T1` | `02 Task Creation` | `Unassigned` | `Open` badge | `% Completion 20%`), working inline task creation row (`Add Task` + `Suggestions` AI icon), and inbound task email alias link (`To add Task via email to this Task list 📋`).
  ...
  - **In-Module Task Automation Drawer & Rule Builder**: Clicking top-right **`Automation`** button slides out the Task Automation Drawer right inside the Tasks module, providing rule status toggles, rule reordering, edit/delete actions, and a visual flow canvas builder for creating/editing workflow rules without leaving the Tasks screen.
  - **Import Tasks Wizard Modal**: Top-right dropdown option opening 4-step import wizard matching Screenshot 1 with file format selection cards (*MPP/MPX* cyan card, *XLS/XLSX*, *JSON*, *CSV*), file upload box displaying uploaded file (*mytasks.mpp.textClipping*) with trash delete icon 🗑️, 20 MB size limit note, *Associate Blueprint* checkbox, and *[ Import ]* orange action button.
  - **Subtasks Hierarchy & Drag-and-Drop Reordering Engine**: Supports up to 6 levels of nested subtasks, inline reorder handle `::` next to task ID (`COAF-T52`), real-time drag-and-drop reordering with blue drop target banner **`☑ Drop here to reorder the task.`** matching Screenshot 2, Gantt chart subtask timelines, and timeline/hours roll-up calculation to parent tasks.
  - **Critical Path & Slack Engine (`gantt-chart.tsx`)**:
    - **Critical Path Calculation Engine**: Identifies the longest sequence of dependent tasks (zero float/slack) determining overall project duration. Renders critical task bars in bold bright red (`fill="#EF4444"`) with `🔴 CRITICAL` indicators.
    - **Smart Bar Toolbar Controls**: Top-right Gantt Smart Bar toggles for `Critical Path` (red pulse badge) and `Slack (Float)` (blue status pill).
    - **Dotted Slack (Float) Lines**: Non-critical tasks render **dotted extension lines** (`strokeDasharray="4 4"`) displaying exact slack duration (`Slack: Xd`) between predecessor completion and successor start dates across FS, SS, FF, and SF dependencies.
